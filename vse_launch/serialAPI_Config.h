...
// #endif /* SBGC_USE_LINUX_DRIVER */
    ...
    /* Include unistd.h to use the access() function for file checking */
    #include <unistd.h>

    /*  Attention!
        The serial port must have extended rights:
        sudo usermod -a -G dialout $USER
        (Re-login required after running this command)
     */
    /** @cond
     */
    ...
    /* 
       ...
       Checks if /dev/ttyUSB0 exists. If yes, uses it. 
       Otherwise, falls back to /dev/ttyUSB1.
    */
    #define SBGC_SERIAL_PORT  (access("/dev/ttyUSB0", F_OK) == 0 ? "/dev/ttyUSB0" : "/dev/ttyUSB1")
