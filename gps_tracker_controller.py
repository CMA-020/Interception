...
class GPSStrikeController(Node):
    ...
    def __init__(self, target_lat, target_lon, target_relative_alt=750.0):
        ...
        self.TRACK_THRUST = 0.65

        self.pid_roll = SimplePID(kp=0.3, ki=0.001, kd=0.2, max_output=1.5)
        self.pid_pitch = SimplePID(kp=0.5, ki=0.001, kd=0.1, max_output=1.5)
        self.pid_yaw = SimplePID(kp=1.5, ki=0.001, kd=0.5, max_output=1.2) 
    ...
    def tracker_ang(self):
        ...
        if self.track_on and bbox is not None:
            ...
            # MODIFIED: Logging physical telemetry metrics for absolute orientation states
            self.get_logger().info(
            ...
            # MODIFIED: Included YAW P_err, I_err, and D_err lines inside standard prints
            self.get_logger().info(
            ...
            # Keeping yaw command deactivated as requested by original system spec
